...
func main() {
	...
	mux := mux.NewRouter()
	mux.HandleFunc("/", dfs.HTTPHandlerFunc(indexHandler)).Methods("GET", "HEAD")
	mux.HandleFunc("/v1/ping", dfs.HTTPHandlerFunc(pingHandler)).Methods("GET", "HEAD")
	mux.HandleFunc("/v1", dfs.HTTPHandlerFunc(fetchHandler)).Methods("GET", "HEAD")
	mux.HandleFunc("/v1", dfs.HTTPHandlerFunc(updateHandler)).Methods("POST", "PUT")
	mux.HandleFunc("/v1", dfs.HTTPHandlerFunc(deleteHandler)).Methods("DELETE")
	mux.HandleFunc("/v1/stats", dfs.HTTPHandlerFunc(privateStatsHandler)).Methods("GET")
	mux.HandleFunc("/v1/flush", dfs.HTTPHandlerFunc(flushHandler)).Methods("DELETE")
	mux.HandleFunc("/v1/bulk", dfs.HTTPHandlerFunc(bulkHandler)).Methods("POST", "PUT")
	mux.HandleFunc("/login", dfs.HTTPHandlerFunc(handleGitHubLogin)).Methods("GET")
	mux.HandleFunc("/logout", dfs.HTTPHandlerFunc(logoutHandler)).Methods("GET", "POST")
	mux.HandleFunc("/github_oauth_cb", dfs.HTTPHandlerFunc(handleGitHubCallback)).Methods("GET")
	mux.HandleFunc("/domainkeys/new", dfs.HTTPHandlerFunc(domainkeyNewHandler)).Methods("POST")
	mux.HandleFunc("/domainkeys/delete", dfs.HTTPHandlerFunc(domainkeyDeleteHandler)).Methods("POST")
